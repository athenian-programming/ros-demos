...
def callback(msg):
    ...
    # Map left joystick
        ...
        target_linear_vel = target_linear_vel + (LINEAR_ADJ * msg.axes[1])
        print vels("Forward   ", target_linear_vel, target_ang_vel)
    elif msg.axes[1] < 0.0:
        target_linear_vel = target_linear_vel - (LINEAR_ADJ * -msg.axes[1])
        print vels("Backward   ", target_linear_vel, target_ang_vel)
    elif msg.axes[0] > 0.0:
        target_ang_vel = target_ang_vel + (ANG_ADJ * msg.axes[0])
        print vels("Left    ", target_linear_vel, target_ang_vel)
    elif msg.axes[0] < 0.0:
        target_ang_vel = target_ang_vel - (ANG_ADJ * -msg.axes[0])
        print vels("Right   ", target_linear_vel, target_ang_vel)
    ...
    twist.angular.x = 0
    twist.angular.y = 0
    twist.angular.z = bound(-1.0, 1.0, control_ang_vel)
    pub.publish(twist)
